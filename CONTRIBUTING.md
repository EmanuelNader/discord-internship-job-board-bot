...
Edit `src/config/roles.config.ts` (channel name, emoji, Discord role name, `enabled`). `/settings` lists those values in Discord. `/setup` and `/onboard` apply them; joining or restarting does not create channels. `/onboard` posts the reaction panel in `#job-board`, not in the channel you typed the command in. Set `enabled: false` to skip that family’s channel, onboard reaction, `/role` option, and posting.
